...
#' @description myadmin-client-r.SSLCertificates
#'
...
SSLCertificatesApi <- R6::R6Class(
  ...
  public = list(
    ...
    get_ssl_invoices = function(id, ...){
      args <- list(...)
      queryParams <- list()
      headerParams <- character()

      urlPath <- "/ssl/{id}/invoices"
      if (!missing(`id`)) {
        urlPath <- gsub(paste0("\\{", "id", "\\}"), `id`, urlPath)
      }
    ...
    get_ssl_welcome_email = function(id, ...){
      args <- list(...)
      queryParams <- list()
      headerParams <- character()

      urlPath <- "/ssl/{id}/welcome_email"
      if (!missing(`id`)) {
        urlPath <- gsub(paste0("\\{", "id", "\\}"), `id`, urlPath)
      }
