  ...
  class CreateFirewallRule
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::CreateFirewallRule` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::CreateFirewallRule`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'protocol_id')
        self.protocol_id = attributes[:'protocol_id']
      end

      if attributes.key?(:'xdp_action')
        self.xdp_action = attributes[:'xdp_action']
      end
    end
    ...
    def list_invalid_properties
      ...
      if @protocol_id.nil?
        invalid_properties.push('invalid value for "protocol_id", protocol_id cannot be nil.')
      end

      if @xdp_action.nil?
        invalid_properties.push('invalid value for "xdp_action", xdp_action cannot be nil.')
      end
    ...
    def valid?
      ...
      protocol_id_validator = EnumAttributeValidator.new('Object', [1, 2])
      return false unless protocol_id_validator.valid?(@protocol_id)
      return false if @xdp_action.nil?
      xdp_action_validator = EnumAttributeValidator.new('Object', [0, 1])
    ...
    def hash
      [destination_port, source_ip, source_port, protocol_id, xdp_action].hash
    end
