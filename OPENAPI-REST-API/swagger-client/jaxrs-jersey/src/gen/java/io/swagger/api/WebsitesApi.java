...
import io.swagger.api.factories.WebsitesApiServiceFactory;
...
public class WebsitesApi  {
   private final WebsitesApiService delegate;

   public WebsitesApi(@Context ServletConfig servletContext) {
      WebsitesApiService delegate = null;

      if (servletContext != null) {
         String implClass = servletContext.getInitParameter("WebsitesApi.implementation");
         if (implClass != null && !"".equals(implClass.trim())) {
            try {
               delegate = (WebsitesApiService) Class.forName(implClass).newInstance();
            } catch (Exception e) {
      ...
      if (delegate == null) {
         delegate = WebsitesApiServiceFactory.getWebsitesApi();
      }
