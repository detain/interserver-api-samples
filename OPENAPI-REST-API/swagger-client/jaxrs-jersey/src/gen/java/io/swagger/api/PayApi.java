...
public class PayApi  {
   private final PayApiService delegate;

   public PayApi(@Context ServletConfig servletContext) {
      PayApiService delegate = null;

      if (servletContext != null) {
         String implClass = servletContext.getInitParameter("PayApi.implementation");
         if (implClass != null && !"".equals(implClass.trim())) {
            try {
               delegate = (PayApiService) Class.forName(implClass).newInstance();
            } catch (Exception e) {
      ...
      if (delegate == null) {
         delegate = PayApiServiceFactory.getPayApi();
      }
