...
import io.swagger.api.factories.ScrubIpsApiServiceFactory;
...
public class ScrubIpsApi  {
   private final ScrubIpsApiService delegate;

   public ScrubIpsApi(@Context ServletConfig servletContext) {
      ScrubIpsApiService delegate = null;

      if (servletContext != null) {
         String implClass = servletContext.getInitParameter("ScrubIpsApi.implementation");
         if (implClass != null && !"".equals(implClass.trim())) {
            try {
               delegate = (ScrubIpsApiService) Class.forName(implClass).newInstance();
            } catch (Exception e) {
      ...
      if (delegate == null) {
         delegate = ScrubIpsApiServiceFactory.getScrubIpsApi();
      }
