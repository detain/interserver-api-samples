...
import io.swagger.api.factories.SslApiServiceFactory;
...
public class SslApi  {
   private final SslApiService delegate;

   public SslApi(@Context ServletConfig servletContext) {
      SslApiService delegate = null;

      if (servletContext != null) {
         String implClass = servletContext.getInitParameter("SslApi.implementation");
         if (implClass != null && !"".equals(implClass.trim())) {
            try {
               delegate = (SslApiService) Class.forName(implClass).newInstance();
            } catch (Exception e) {
      ...
      if (delegate == null) {
         delegate = SslApiServiceFactory.getSslApi();
      }
