...
import io.swagger.api.factories.CaptchaApiServiceFactory;
...
public class CaptchaApi  {
   private final CaptchaApiService delegate;

   public CaptchaApi(@Context ServletConfig servletContext) {
      CaptchaApiService delegate = null;

      if (servletContext != null) {
         String implClass = servletContext.getInitParameter("CaptchaApi.implementation");
         if (implClass != null && !"".equals(implClass.trim())) {
            try {
               delegate = (CaptchaApiService) Class.forName(implClass).newInstance();
            } catch (Exception e) {
      ...
      if (delegate == null) {
         delegate = CaptchaApiServiceFactory.getCaptchaApi();
      }
