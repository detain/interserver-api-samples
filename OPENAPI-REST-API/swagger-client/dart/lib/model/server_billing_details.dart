...
class ServerBillingDetails {
  ...
  @override
  String toString() {
    return 'ServerBillingDetails[serviceLastInvoiceDate=$serviceLastInvoiceDate, servicePaymentStatus=$servicePaymentStatus, serviceFrequency=$serviceFrequency, nextDate=$nextDate, serviceNextInvoiceDate=$serviceNextInvoiceDate, serviceCurrency=$serviceCurrency, serviceCurrencySymbol=$serviceCurrencySymbol, serviceCostInfo=$serviceCostInfo, serviceExtra=$serviceExtra, serviceExtraJson=$serviceExtraJson, ]';
  }

  ServerBillingDetails.fromJson(Map<String, dynamic> json) {
    if (json == null) return;
  ...
  static List<ServerBillingDetails> listFromJson(List<dynamic> json) {
    return json == null ? new List<ServerBillingDetails>() : json.map((value) => new ServerBillingDetails.fromJson(value)).toList();
  }

  static Map<String, ServerBillingDetails> mapFromJson(Map<String, Map<String, dynamic>> json) {
    var map = new Map<String, ServerBillingDetails>();
    if (json != null && json.length > 0) {
      json.forEach((String key, Map<String, dynamic> value) => map[key] = new ServerBillingDetails.fromJson(value));
    }
    return map;
  }
}
