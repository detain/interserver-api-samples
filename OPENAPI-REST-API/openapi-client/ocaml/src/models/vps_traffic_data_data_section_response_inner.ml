 ...
 *)
        ...
        let of_yojson json =
          ...
          |> function
             | [t] -> Ok t
             | [] -> Error ("Failed to parse JSON " ^ Yojson.Safe.show json ^ " into a value of type Vps_traffic_data_data_section_response_inner.t")
             | ts -> let parsed_ts = ts
                         |> Stdlib.List.map show
                         |> Stdlib.String.concat " | "
                     in Error ("Failed to parse JSON " ^ Yojson.Safe.show json ^ " into a value of type Vps_traffic_data_data_section_response_inner.t: oneOf should only succeed on one parser, but the JSON was parsed into [" ^ parsed_ts ^ "]")
