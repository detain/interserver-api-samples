...
class DomainServiceType {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is DomainServiceType &&
  ...
  @override
  String toString() => 'DomainServiceType[servicesId=$servicesId, servicesName=$servicesName, servicesCost=$servicesCost, servicesCategory=$servicesCategory, servicesBuyable=$servicesBuyable, servicesType=$servicesType, servicesField1=$servicesField1, servicesField2=$servicesField2, servicesModule=$servicesModule]';
  ...
  /// Returns a new [DomainServiceType] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static DomainServiceType? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "DomainServiceType[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "DomainServiceType[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<DomainServiceType> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <DomainServiceType>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = DomainServiceType.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, DomainServiceType> mapFromJson(dynamic json) {
    final map = <String, DomainServiceType>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = DomainServiceType.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of DomainServiceType-objects as value to a dart map
  static Map<String, List<DomainServiceType>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<DomainServiceType>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = DomainServiceType.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
