...
class ServerOrderRAID {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is ServerOrderRAID &&
    other.id == id &&
    other.price == price &&
    other.img == img &&
  ...
  @override
  String toString() => 'ServerOrderRAID[id=$id, price=$price, img=$img, shortDesc=$shortDesc, longDesc=$longDesc, monthlyPrice=$monthlyPrice, active=$active, priceDisplay=$priceDisplay, monthlyPriceDisplay=$monthlyPriceDisplay]';
  ...
  /// Returns a new [ServerOrderRAID] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static ServerOrderRAID? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "ServerOrderRAID[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "ServerOrderRAID[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return ServerOrderRAID(
  ...
  static List<ServerOrderRAID> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <ServerOrderRAID>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = ServerOrderRAID.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, ServerOrderRAID> mapFromJson(dynamic json) {
    final map = <String, ServerOrderRAID>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = ServerOrderRAID.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of ServerOrderRAID-objects as value to a dart map
  static Map<String, List<ServerOrderRAID>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<ServerOrderRAID>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = ServerOrderRAID.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
