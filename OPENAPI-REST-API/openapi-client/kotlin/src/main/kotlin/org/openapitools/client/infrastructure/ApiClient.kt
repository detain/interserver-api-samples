...
open class ApiClient(val baseUrl: String, val client: Call.Factory = defaultClient) {
    ...
    protected fun <T> updateAuthParams(requestConfig: RequestConfig<T>) {
        ...
        if (requestConfig.headers["sessionid"].isNullOrEmpty()) {
            if (apiKey["sessionid"] != null) {
                if (apiKeyPrefix["sessionid"] != null) {
                    requestConfig.headers["sessionid"] = apiKeyPrefix["sessionid"]!! + " " + apiKey["sessionid"]!!
                } else {
                    requestConfig.headers["sessionid"] = apiKey["sessionid"]!!
                }
            }
        }
            if (apiKey["sessionid"] != null) {
