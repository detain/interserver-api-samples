...
function Initialize-QueueResponse {
    [CmdletBinding()]
    Param (
    ...
    Process {
        'Creating PSCustomObject: PSOpenAPITools => QueueResponse' | Write-Debug
        ...
        if ($null -eq $QueueId) {
            throw "invalid value for 'QueueId', 'QueueId' cannot be null."
        }
...
QueueResponse<PSCustomObject>
#>
function ConvertFrom-JsonToQueueResponse {
    Param(
        [AllowEmptyString()]
        [string]$Json
    )
    ...
    Process {
        'Converting JSON to PSCustomObject: PSOpenAPITools => QueueResponse' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        $JsonParameters = ConvertFrom-Json -InputObject $Json

        # check if Json contains properties not defined in QueueResponse
        $AllProperties = ("text", "queueId")
        ...
        if (!([bool]($JsonParameters.PSobject.Properties.name -match "queueId"))) {
            throw "Error! JSON cannot be serialized due to the required property 'queueId' missing."
        } else {
