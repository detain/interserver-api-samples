...
object LoginErrorResponse {
  given encoderLoginErrorResponse: Encoder[LoginErrorResponse] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
        t.message.map(v => "message" -> v.asJson),
        t.field.map(v => "field" -> v.asJson)
      ).flatten
    }
  }
  given decoderLoginErrorResponse: Decoder[LoginErrorResponse] = Decoder.instance { c =>
    for {
      message <- c.downField("message").as[Option[String]]
      field <- c.downField("field").as[Option[String]]
    } yield LoginErrorResponse(
      message = message,
      field = field
    )
  }
