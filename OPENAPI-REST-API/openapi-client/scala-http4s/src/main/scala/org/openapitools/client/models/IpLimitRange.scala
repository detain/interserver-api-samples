...
object IpLimitRange {
  given encoderIpLimitRange: Encoder[IpLimitRange] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
        Some("start" -> t.start.asJson),
        Some("end" -> t.end.asJson)
      ).flatten
    }
  }
  given decoderIpLimitRange: Decoder[IpLimitRange] = Decoder.instance { c =>
    for {
