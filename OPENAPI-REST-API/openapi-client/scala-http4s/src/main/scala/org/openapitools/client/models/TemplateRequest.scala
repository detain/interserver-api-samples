...
object TemplateRequest {
  given encoderTemplateRequest: Encoder[TemplateRequest] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
        Some("template" -> t.template.asJson),
        Some("localPassword" -> t.localPassword.asJson),
        t.password.map(v => "password" -> v.asJson)
      ).flatten
    }
  }
  given decoderTemplateRequest: Decoder[TemplateRequest] = Decoder.instance { c =>
    for {
      template <- c.downField("template").as[String]
      localPassword <- c.downField("localPassword").as[String]
      password <- c.downField("password").as[Option[String]]
    } yield TemplateRequest(
      template = template,
