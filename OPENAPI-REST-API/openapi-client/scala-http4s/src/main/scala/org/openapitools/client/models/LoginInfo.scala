...
object LoginInfo {
  given encoderLoginInfo: Encoder[LoginInfo] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
        Some("captcha" -> t.captcha.asJson),
        Some("counts" -> t.counts.asJson),
        t.logo.map(v => "logo" -> v.asJson),
        t.language.map(v => "language" -> v.asJson)
      ).flatten
    }
  }
  given decoderLoginInfo: Decoder[LoginInfo] = Decoder.instance { c =>
    for {
      captcha <- c.downField("captcha").as[String]
      counts <- c.downField("counts").as[LoginServiceCounts]
      logo <- c.downField("logo").as[Option[String]]
