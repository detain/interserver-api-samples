...
object BillingPrepayRequest {
  given encoderBillingPrepayRequest: Encoder[BillingPrepayRequest] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
        t.module.map(v => "module" -> v.asJson),
        t.amount.map(v => "amount" -> v.asJson),
        t.automatic_use.map(v => "automatic_use" -> v.asJson)
      ).flatten
    }
  }
  given decoderBillingPrepayRequest: Decoder[BillingPrepayRequest] = Decoder.instance { c =>
    for {
      module <- c.downField("module").as[Option[String]]
      amount <- c.downField("amount").as[Option[BigDecimal]]
      automatic_use <- c.downField("automatic_use").as[Option[String]]
    } yield BillingPrepayRequest(
      module = module,
      amount = amount,
