...
object PatchOauthTwoFactorRequest {
  given encoderPatchOauthTwoFactorRequest: Encoder[PatchOauthTwoFactorRequest] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
        Some("account_id" -> t.account_id.asJson),
        Some("code" -> t.code.asJson)
      ).flatten
    }
  }
  given decoderPatchOauthTwoFactorRequest: Decoder[PatchOauthTwoFactorRequest] = Decoder.instance { c =>
    for {
      account_id <- c.downField("account_id").as[Int]
      code <- c.downField("code").as[String]
    } yield PatchOauthTwoFactorRequest(
