...
object ServerAssets {
  given encoderServerAssets: Encoder[ServerAssets] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
  ...
  given decoderServerAssets: Decoder[ServerAssets] = Decoder.instance { c =>
    for {
      title <- c.downField("title").as[Option[String]]
      size <- c.downField("size").as[Option[Int]]
      `type` <- c.downField("type").as[Option[String]]
      header <- c.downField("header").as[Option[Seq[String]]]
      rows <- c.downField("rows").as[Option[Seq[Seq[String]]]]
    } yield ServerAssets(
      title = title,
      size = size,
      `type` = `type`,
