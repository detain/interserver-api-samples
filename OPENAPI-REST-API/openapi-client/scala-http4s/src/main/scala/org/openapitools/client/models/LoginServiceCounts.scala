...
object LoginServiceCounts {
  given encoderLoginServiceCounts: Encoder[LoginServiceCounts] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
        Some("vps" -> t.vps.asJson),
        Some("websites" -> t.websites.asJson),
        Some("servers" -> t.servers.asJson)
      ).flatten
    }
  }
  given decoderLoginServiceCounts: Decoder[LoginServiceCounts] = Decoder.instance { c =>
    for {
      vps <- c.downField("vps").as[Int]
      websites <- c.downField("websites").as[Int]
      servers <- c.downField("servers").as[Int]
    } yield LoginServiceCounts(
      vps = vps,
      websites = websites,
      servers = servers
    )
