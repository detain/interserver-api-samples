...
object BackupLoginResponse {
  given encoderBackupLoginResponse: Encoder[BackupLoginResponse] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
  ...
  given decoderBackupLoginResponse: Decoder[BackupLoginResponse] = Decoder.instance { c =>
    for {
      success <- c.downField("success").as[Option[Boolean]]
      text <- c.downField("text").as[Option[String]]
    } yield BackupLoginResponse(
      success = success,
      text = text
    )
  }
