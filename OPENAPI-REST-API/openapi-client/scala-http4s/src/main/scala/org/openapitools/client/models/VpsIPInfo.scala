...
object VpsIPInfo {
  given encoderVpsIPInfo: Encoder[VpsIPInfo] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
  ...
  given decoderVpsIPInfo: Decoder[VpsIPInfo] = Decoder.instance { c =>
    for {
      title <- c.downField("title").as[Option[String]]
      rows <- c.downField("rows").as[Option[Seq[VpsIPInfoRow]]]
    } yield VpsIPInfo(
      title = title,
      rows = rows
    )
