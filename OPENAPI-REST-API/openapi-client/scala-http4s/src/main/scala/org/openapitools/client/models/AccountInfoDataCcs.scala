...
object AccountInfoDataCcs {
  given encoderAccountInfoDataCcs: Encoder[AccountInfoDataCcs] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
      ).flatten
    }
  }
  given decoderAccountInfoDataCcs: Decoder[AccountInfoDataCcs] = Decoder.instance { c =>
    for {
    } yield AccountInfoDataCcs(
    )
  }
}
