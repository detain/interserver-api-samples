...
object VpsTrafficUsageAverageSectionResponse {
  given encoderVpsTrafficUsageAverageSectionResponse: Encoder[VpsTrafficUsageAverageSectionResponse] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
        Some("total" -> t.total.asJson),
        Some("count" -> t.count.asJson),
        Some("value" -> t.value.asJson)
      ).flatten
    }
  }
  given decoderVpsTrafficUsageAverageSectionResponse: Decoder[VpsTrafficUsageAverageSectionResponse] = Decoder.instance { c =>
    for {
      total <- c.downField("total").as[Int]
      count <- c.downField("count").as[Int]
      value <- c.downField("value").as[Int]
    } yield VpsTrafficUsageAverageSectionResponse(
      total = total,
      count = count,
      value = value
    )
