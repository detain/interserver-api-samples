...
object WebsiteLoginResponse {
  given encoderWebsiteLoginResponse: Encoder[WebsiteLoginResponse] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
  ...
  given decoderWebsiteLoginResponse: Decoder[WebsiteLoginResponse] = Decoder.instance { c =>
    for {
      `type` <- c.downField("type").as[Option[String]]
      location <- c.downField("location").as[Option[String]]
    } yield WebsiteLoginResponse(
      `type` = `type`,
      location = location
    )
  }
