...
object QueueResponse {
  given encoderQueueResponse: Encoder[QueueResponse] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
        Some("text" -> t.text.asJson),
        Some("queueId" -> t.queueId.asJson)
      ).flatten
    }
  }
  given decoderQueueResponse: Decoder[QueueResponse] = Decoder.instance { c =>
    for {
      text <- c.downField("text").as[String]
      queueId <- c.downField("queueId").as[Int]
    } yield QueueResponse(
      text = text,
      queueId = queueId
    )
