...
object BackupIPInfoRow {
  given encoderBackupIPInfoRow: Encoder[BackupIPInfoRow] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
  ...
  given decoderBackupIPInfoRow: Decoder[BackupIPInfoRow] = Decoder.instance { c =>
    for {
      desc <- c.downField("desc").as[Option[String]]
      value <- c.downField("value").as[Option[String]]
    } yield BackupIPInfoRow(
      desc = desc,
      value = value
    )
  }
