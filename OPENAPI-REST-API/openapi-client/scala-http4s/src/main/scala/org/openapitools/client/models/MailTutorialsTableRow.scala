...
object MailTutorialsTableRow {
  given encoderMailTutorialsTableRow: Encoder[MailTutorialsTableRow] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
  ...
  given decoderMailTutorialsTableRow: Decoder[MailTutorialsTableRow] = Decoder.instance { c =>
    for {
      desc <- c.downField("desc").as[Option[String]]
      value <- c.downField("value").as[Option[String]]
    } yield MailTutorialsTableRow(
      desc = desc,
      value = value
    )
  }
