...
object PostWebsiteMigration200Response {
  given encoderPostWebsiteMigration200Response: Encoder[PostWebsiteMigration200Response] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
  ...
  given decoderPostWebsiteMigration200Response: Decoder[PostWebsiteMigration200Response] = Decoder.instance { c =>
    for {
      text <- c.downField("text").as[Option[String]]
      ticket <- c.downField("ticket").as[Option[Int]]
    } yield PostWebsiteMigration200Response(
      text = text,
      ticket = ticket
    )
  }
