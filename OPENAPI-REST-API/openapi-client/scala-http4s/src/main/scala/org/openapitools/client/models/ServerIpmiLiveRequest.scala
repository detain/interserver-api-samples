...
object ServerIpmiLiveRequest {
  given encoderServerIpmiLiveRequest: Encoder[ServerIpmiLiveRequest] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
        Some("ip" -> t.ip.asJson),
  ...
  given decoderServerIpmiLiveRequest: Decoder[ServerIpmiLiveRequest] = Decoder.instance { c =>
    for {
      ip <- c.downField("ip").as[String]
      asset <- c.downField("asset").as[Option[Int]]
    } yield ServerIpmiLiveRequest(
      ip = ip,
      asset = asset
    )
  }
