...
object ServerLease {
  given encoderServerLease: Encoder[ServerLease] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
        Some("mac" -> t.mac.asJson),
        Some("authenticated" -> t.authenticated.asJson),
        Some("group" -> t.group.asJson)
      ).flatten
    }
  }
  given decoderServerLease: Decoder[ServerLease] = Decoder.instance { c =>
    for {
      mac <- c.downField("mac").as[String]
      authenticated <- c.downField("authenticated").as[Boolean]
