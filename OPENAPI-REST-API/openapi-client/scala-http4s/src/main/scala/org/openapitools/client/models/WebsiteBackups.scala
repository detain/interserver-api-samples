...
object WebsiteBackups {
  given encoderWebsiteBackups: Encoder[WebsiteBackups] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
      ).flatten
    }
  }
  given decoderWebsiteBackups: Decoder[WebsiteBackups] = Decoder.instance { c =>
    for {
    } yield WebsiteBackups(
    )
  }
}
