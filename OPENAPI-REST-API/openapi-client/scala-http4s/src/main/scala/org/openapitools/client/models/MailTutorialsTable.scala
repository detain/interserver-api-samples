...
object MailTutorialsTable {
  given encoderMailTutorialsTable: Encoder[MailTutorialsTable] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
  ...
  given decoderMailTutorialsTable: Decoder[MailTutorialsTable] = Decoder.instance { c =>
    for {
      title <- c.downField("title").as[Option[String]]
      rows <- c.downField("rows").as[Option[Seq[MailTutorialsTableRow]]]
    } yield MailTutorialsTable(
      title = title,
      rows = rows
    )
  }
