...
object RestoreRequest {
  given encoderRestoreRequest: Encoder[RestoreRequest] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
        t.backup.map(v => "backup" -> v.asJson),
        t.password.map(v => "password" -> v.asJson)
      ).flatten
    }
  }
  given decoderRestoreRequest: Decoder[RestoreRequest] = Decoder.instance { c =>
    for {
      backup <- c.downField("backup").as[Option[String]]
      password <- c.downField("password").as[Option[String]]
    } yield RestoreRequest(
      backup = backup,
      password = password
    )
  }
