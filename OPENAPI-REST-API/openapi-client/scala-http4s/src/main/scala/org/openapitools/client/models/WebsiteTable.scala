...
object WebsiteTable {
  given encoderWebsiteTable: Encoder[WebsiteTable] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
  ...
  given decoderWebsiteTable: Decoder[WebsiteTable] = Decoder.instance { c =>
    for {
      title <- c.downField("title").as[Option[String]]
      rows <- c.downField("rows").as[Option[Seq[WebsiteTableRow]]]
    } yield WebsiteTable(
      title = title,
      rows = rows
    )
