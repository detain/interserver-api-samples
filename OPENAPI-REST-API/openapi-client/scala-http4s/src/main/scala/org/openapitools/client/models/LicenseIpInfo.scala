...
object LicenseIpInfo {
  given encoderLicenseIpInfo: Encoder[LicenseIpInfo] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
  ...
  given decoderLicenseIpInfo: Decoder[LicenseIpInfo] = Decoder.instance { c =>
    for {
      title <- c.downField("title").as[Option[String]]
      rows <- c.downField("rows").as[Option[Seq[LicenseIpInfoRow]]]
    } yield LicenseIpInfo(
      title = title,
      rows = rows
    )
