...
object MailExtraInfoTable {
  given encoderMailExtraInfoTable: Encoder[MailExtraInfoTable] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
  ...
  given decoderMailExtraInfoTable: Decoder[MailExtraInfoTable] = Decoder.instance { c =>
    for {
      title <- c.downField("title").as[Option[String]]
      rows <- c.downField("rows").as[Option[Seq[MailExtraInfoTableRow]]]
    } yield MailExtraInfoTable(
      title = title,
      rows = rows
    )
  }
