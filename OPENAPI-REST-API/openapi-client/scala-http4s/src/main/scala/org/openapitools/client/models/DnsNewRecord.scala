...
object DnsNewRecord {
  given encoderDnsNewRecord: Encoder[DnsNewRecord] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
  ...
  given decoderDnsNewRecord: Decoder[DnsNewRecord] = Decoder.instance { c =>
    for {
      name <- c.downField("name").as[String]
      `type` <- c.downField("type").as[DnsRecordType]
      content <- c.downField("content").as[String]
      ttl <- c.downField("ttl").as[Option[Int]]
      prio <- c.downField("prio").as[Option[Int]]
    } yield DnsNewRecord(
      name = name,
      `type` = `type`,
      content = content,
