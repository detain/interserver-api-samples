...
object VpsSnapshot {
  given encoderVpsSnapshot: Encoder[VpsSnapshot] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
        t.name.map(v => "name" -> v.asJson),
        t.used.map(v => "used" -> v.asJson),
        t.date.map(v => "date" -> v.asJson)
      ).flatten
    }
  }
  given decoderVpsSnapshot: Decoder[VpsSnapshot] = Decoder.instance { c =>
    for {
      name <- c.downField("name").as[Option[String]]
      used <- c.downField("used").as[Option[Int]]
      date <- c.downField("date").as[Option[Int]]
    } yield VpsSnapshot(
      name = name,
      used = used,
      date = date
