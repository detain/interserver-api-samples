...
object BackupIPInfo {
  given encoderBackupIPInfo: Encoder[BackupIPInfo] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
  ...
  given decoderBackupIPInfo: Decoder[BackupIPInfo] = Decoder.instance { c =>
    for {
      title <- c.downField("title").as[Option[String]]
      rows <- c.downField("rows").as[Option[Seq[BackupIPInfoRow]]]
    } yield BackupIPInfo(
      title = title,
      rows = rows
    )
