...
case class BillingInvoiceList(
    rows: Option[Seq[Json]] = None,
    summary: Option[Json] = None
)
  
object BillingInvoiceList {
  given encoderBillingInvoiceList: Encoder[BillingInvoiceList] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
        t.rows.map(v => "rows" -> v.asJson),
        t.summary.map(v => "summary" -> v.asJson)
      ).flatten
    }
  }
  given decoderBillingInvoiceList: Decoder[BillingInvoiceList] = Decoder.instance { c =>
    for {
      rows <- c.downField("rows").as[Option[Seq[Json]]]
      summary <- c.downField("summary").as[Option[Json]]
    } yield BillingInvoiceList(
      rows = rows,
      summary = summary
    )
  }
