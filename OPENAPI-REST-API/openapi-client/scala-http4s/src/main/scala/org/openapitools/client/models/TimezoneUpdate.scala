...
object TimezoneUpdate {
  given encoderTimezoneUpdate: Encoder[TimezoneUpdate] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
  ...
  given decoderTimezoneUpdate: Decoder[TimezoneUpdate] = Decoder.instance { c =>
    for {
      timezone <- c.downField("timezone").as[String]
    } yield TimezoneUpdate(
      timezone = timezone
    )
  }
}
