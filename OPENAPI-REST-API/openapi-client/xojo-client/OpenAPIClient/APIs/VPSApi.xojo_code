...
Protected Class VPSApi
	...
	#tag Method, Flags = &h0
		Sub PostVpsBackup(, id As Integer)
		  // Operation postVpsBackup
		  // Start a VPS Backup
		  // - 
		  // - parameter id: (path) VPS ID number 
		  //
		  // Invokes VPSApiCallbackHandler.PostVpsBackupCallback(QueueResponse) on completion. 
		  //
		  // - GET /vps/{id}/backup
		  ...
		  If me.ApiKeysessionIdCookieAuth = "" Then Raise New OpenAPIClient.OpenAPIClientException(kErrorCannotAuthenticate, "API key is unset. Please assign a value to `VPSApi.ApiKeysessionIdCookieAuth` before invoking `VPSApi.PostVpsBackup()`.")
		  
		  
		  If me.ApiKeyapiKeyAuth = "" Then Raise New OpenAPIClient.OpenAPIClientException(kErrorCannotAuthenticate, "API key is unset. Please assign a value to `VPSApi.ApiKeyapiKeyAuth` before invoking `VPSApi.PostVpsBackup()`.")
		  
		  localVarHTTPSocket.SetRequestHeader(EncodeURLComponent("X-API-KEY"), EncodeURLComponent(me.ApiKeyapiKeyAuth))
		  If me.ApiKeysessionIdHeaderAuth = "" Then Raise New OpenAPIClient.OpenAPIClientException(kErrorCannotAuthenticate, "API key is unset. Please assign a value to `VPSApi.ApiKeysessionIdHeaderAuth` before invoking `VPSApi.PostVpsBackup()`.")
		  ...
		  AddHandler localVarHTTPSocket.PageReceived, addressof me.PostVpsBackup_handler
		  AddHandler localVarHTTPSocket.Error, addressof Me.PostVpsBackup_error
	...
	#tag Method, Flags = &h21
		Private Function PostVpsBackupPrivateFuncDeserializeResponse(HTTPStatus As Integer, Headers As InternetHeaders, error As OpenAPIClient.OpenAPIClientException, Content As String, ByRef outData As OpenAPIClient.Models.QueueResponse) As Boolean
		  Dim contentType As String = Headers.Value("Content-Type")
		  Dim contentEncoding As TextEncoding = OpenAPIClient.EncodingFromContentType(contentType)
		  Content = DefineEncoding(Content, contentEncoding)
	...
	#tag Method, Flags = &h21
		Private Sub PostVpsBackup_error(sender As HTTPSecureSocket, Code As Integer)
		  If sender <> nil Then sender.Close()

		  Dim error As New OpenAPIClient.OpenAPIClientException(Code)
		  Dim data As OpenAPIClient.Models.QueueResponse
		  CallbackHandler.PostVpsBackupCallback(error, data)
		End Sub
	#tag EndMethod

	#tag Method, Flags = &h21
		Private Sub PostVpsBackup_handler(sender As HTTPSecureSocket, URL As String, HTTPStatus As Integer, Headers As InternetHeaders, Content As String)
		  #Pragma Unused URL
		  ...
		  Dim data As OpenAPIClient.Models.QueueResponse
		  Call PostVpsBackupPrivateFuncDeserializeResponse(HTTPStatus, Headers, error, Content, data)
