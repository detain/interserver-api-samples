...
local ssl_certificates_api_mt = {
	__name = "ssl_certificates_api";
	__index = ssl_certificates_api;
}

local function new_ssl_certificates_api(authority, basePath, schemes)
	local schemes_map = {}
...
function ssl_certificates_api:add_ssl()
	local req = http_request.new_from_uri({
		...
		host = self.host;
		port = self.port;
		path = string.format("%s/ssl/order",
			self.basePath);
	})
...
function ssl_certificates_api:get_new_ssl()
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
...
function ssl_certificates_api:get_ssl_info(id)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
		path = string.format("%s/ssl/%s",
			self.basePath, id);
	})
...
function ssl_certificates_api:get_ssl_invoices(id)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
		path = string.format("%s/ssl/%s/invoices",
			self.basePath, id);
	})
...
function ssl_certificates_api:get_ssl_list()
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
		path = string.format("%s/ssl",
			self.basePath);
	})
...
function ssl_certificates_api:get_ssl_welcome_email(id)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
		path = string.format("%s/ssl/%s/welcome_email",
			self.basePath, id);
	})
...
function ssl_certificates_api:ssl_cancel(id)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
	...
	if http_status:sub(1,1) == "2" then
		...
		return openapiclient_ssl_cancel_200_response.cast(result), headers
	else
		local body, err, errno2 = stream:get_body_as_string()
		if not body then
			return nil, err, errno2
...
function ssl_certificates_api:update_ssl_info(id)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
