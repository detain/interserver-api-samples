...
# **update_dns_record**
> update_dns_record(domain_id, record_id, name=name, type=type, content=content, ttl=ttl, prio=prio, disabled=disabled, ordername=ordername, auth=auth)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Update DNS Record
        api_instance.update_dns_record(domain_id, record_id, name=name, type=type, content=content, ttl=ttl, prio=prio, disabled=disabled, ordername=ordername, auth=auth)
    except Exception as e:
