//
// DomainSearchResponse.swift
//
// Generated by openapi-generator
// https://openapi-generator.tech
//

import Foundation

/// Lookup and suggestion results returned for a domain search. 
public struct DomainSearchResponse: Codable {
    /// Indicates whether the registrar search succeeded.
    public var success: Bool?
    /// Human-readable status text from the registrar.
    public var responseText: String?
    /// Response time as reported by the registrar.
    public var responseTime: String?
    /// Availability lookup results for queried domains.
    public var lookup: [[String: Any]]?
    /// Suggested alternative domains and availability data.
    public var suggest: [[String: Any]]?
    /// TLDs evaluated during the search.
    public var tlds: [String]?

    public init(success: Bool? = nil, responseText: String? = nil, responseTime: String? = nil, lookup: [[String: Any]]? = nil, suggest: [[String: Any]]? = nil, tlds: [String]? = nil) {
        self.success = success
        self.responseText = responseText
        self.responseTime = responseTime
        self.lookup = lookup
        self.suggest = suggest
        self.tlds = tlds
    }

    public enum CodingKeys: String, CodingKey, CaseIterable {
        case success
        case responseText = "response_text"
        case responseTime = "response_time"
        case lookup
        case suggest
        case tlds
    }

    public init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        success = try container.decodeIfPresent(Bool.self, forKey: .success)
        responseText = try container.decodeIfPresent(String.self, forKey: .responseText)
        responseTime = try container.decodeIfPresent(String.self, forKey: .responseTime)
        lookup = try container.decodeIfPresent([[String: Any]].self, forKey: .lookup)
        suggest = try container.decodeIfPresent([[String: Any]].self, forKey: .suggest)
        tlds = try container.decodeIfPresent([String].self, forKey: .tlds)
    }

    public func encode(to encoder: Encoder) throws {
        var container = encoder.container(keyedBy: CodingKeys.self)
        try container.encodeIfPresent(success, forKey: .success)
        try container.encodeIfPresent(responseText, forKey: .responseText)
        try container.encodeIfPresent(responseTime, forKey: .responseTime)
        try container.encodeIfPresent(lookup, forKey: .lookup)
        try container.encodeIfPresent(suggest, forKey: .suggest)
        try container.encodeIfPresent(tlds, forKey: .tlds)
    }
}
