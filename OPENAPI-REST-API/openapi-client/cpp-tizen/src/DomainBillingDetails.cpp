...
DomainBillingDetails::__init()
{
	...
	//service_extra = new DomainBillingExtra();
	//service_extra_json = std::string();
}

void
DomainBillingDetails::__cleanup()
{
...
void
DomainBillingDetails::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
		...
		if (isprimitive("DomainBillingExtra")) {
			jsonToValue(&service_extra, node, "DomainBillingExtra", "DomainBillingExtra");
		} else {
			
			DomainBillingExtra* obj = static_cast<DomainBillingExtra*> (&service_extra);
...
DomainBillingDetails::DomainBillingDetails(char* json)
{
	this->fromJson(json);
}
