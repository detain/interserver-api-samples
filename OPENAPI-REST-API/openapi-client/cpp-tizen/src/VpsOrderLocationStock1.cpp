#include <map>
#include <cstdlib>
#include <glib-object.h>
#include <json-glib/json-glib.h>
#include "Helpers.h"


#include "VpsOrder_locationStock_1.h"

using namespace std;
using namespace Tizen::ArtikCloud;

VpsOrder_locationStock_1::VpsOrder_locationStock_1()
{
	//__init();
}

VpsOrder_locationStock_1::~VpsOrder_locationStock_1()
{
	//__cleanup();
}

void
VpsOrder_locationStock_1::__init()
{
	//kvm = bool(false);
	//kvmstorage = bool(false);
	//hyperv = bool(false);
}

void
VpsOrder_locationStock_1::__cleanup()
{
	//if(kvm != NULL) {
	//
	//delete kvm;
	//kvm = NULL;
	//}
	//if(kvmstorage != NULL) {
	//
	//delete kvmstorage;
	//kvmstorage = NULL;
	//}
	//if(hyperv != NULL) {
	//
	//delete hyperv;
	//hyperv = NULL;
	//}
	//
}

void
VpsOrder_locationStock_1::fromJson(char* jsonStr)
{
	JsonObject *pJsonObject = json_node_get_object(json_from_string(jsonStr,NULL));
	JsonNode *node;
	const gchar *kvmKey = "kvm";
	node = json_object_get_member(pJsonObject, kvmKey);
	if (node !=NULL) {
	

		if (isprimitive("bool")) {
			jsonToValue(&kvm, node, "bool", "");
		} else {
			
		}
	}
	const gchar *kvmstorageKey = "kvmstorage";
	node = json_object_get_member(pJsonObject, kvmstorageKey);
	if (node !=NULL) {
	

		if (isprimitive("bool")) {
			jsonToValue(&kvmstorage, node, "bool", "");
		} else {
			
		}
	}
	const gchar *hypervKey = "hyperv";
	node = json_object_get_member(pJsonObject, hypervKey);
	if (node !=NULL) {
	

		if (isprimitive("bool")) {
			jsonToValue(&hyperv, node, "bool", "");
		} else {
			
		}
	}
}

VpsOrder_locationStock_1::VpsOrder_locationStock_1(char* json)
{
	this->fromJson(json);
}

char*
VpsOrder_locationStock_1::toJson()
{
	JsonObject *pJsonObject = json_object_new();
	JsonNode *node;
	if (isprimitive("bool")) {
		bool obj = getKvm();
		node = converttoJson(&obj, "bool", "");
	}
	else {
		
	}
	const gchar *kvmKey = "kvm";
	json_object_set_member(pJsonObject, kvmKey, node);
	if (isprimitive("bool")) {
		bool obj = getKvmstorage();
		node = converttoJson(&obj, "bool", "");
	}
	else {
		
	}
	const gchar *kvmstorageKey = "kvmstorage";
	json_object_set_member(pJsonObject, kvmstorageKey, node);
	if (isprimitive("bool")) {
		bool obj = getHyperv();
		node = converttoJson(&obj, "bool", "");
	}
	else {
		
	}
	const gchar *hypervKey = "hyperv";
	json_object_set_member(pJsonObject, hypervKey, node);
	node = json_node_alloc();
	json_node_init(node, JSON_NODE_OBJECT);
	json_node_take_object(node, pJsonObject);
	char * ret = json_to_string(node, false);
	json_node_free(node);
	return ret;
}

bool
VpsOrder_locationStock_1::getKvm()
{
	return kvm;
}

void
VpsOrder_locationStock_1::setKvm(bool  kvm)
{
	this->kvm = kvm;
}

bool
VpsOrder_locationStock_1::getKvmstorage()
{
	return kvmstorage;
}

void
VpsOrder_locationStock_1::setKvmstorage(bool  kvmstorage)
{
	this->kvmstorage = kvmstorage;
}

bool
VpsOrder_locationStock_1::getHyperv()
{
	return hyperv;
}

void
VpsOrder_locationStock_1::setHyperv(bool  hyperv)
{
	this->hyperv = hyperv;
}


