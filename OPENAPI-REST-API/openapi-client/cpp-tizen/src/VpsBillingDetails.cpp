...
VpsBillingDetails::__init()
{
	...
	//service_extra = new VpsServiceExtra();
	//service_extra_json = std::string();
}

void
VpsBillingDetails::__cleanup()
{
...
void
VpsBillingDetails::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
		...
		if (isprimitive("VpsServiceExtra")) {
			jsonToValue(&service_extra, node, "VpsServiceExtra", "VpsServiceExtra");
		} else {
			
			VpsServiceExtra* obj = static_cast<VpsServiceExtra*> (&service_extra);
...
VpsBillingDetails::VpsBillingDetails(char* json)
{
	this->fromJson(json);
}
