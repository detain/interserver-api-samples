...
apiClient_t *apiClient_create() {
    ...
    apiClient->basePath = strdup("https://my.interserver.net/apiv2");
    ...
    apiClient->apiKeys_sessionIdHeaderAuth = NULL;
    apiClient->apiKeys_sessionIdCookieAuth = NULL;
...
, list_t *apiKeys_sessionIdHeaderAuth
, list_t *apiKeys_sessionIdCookieAuth
) {
    apiClient_t *apiClient = malloc(sizeof(apiClient_t));
    if(basePath){
    ...
    if(apiKeys_sessionIdHeaderAuth!= NULL) {
        apiClient->apiKeys_sessionIdHeaderAuth = list_createList();
        listEntry_t *listEntry = NULL;
        list_ForEach(listEntry, apiKeys_sessionIdHeaderAuth) {
            keyValuePair_t *pair = listEntry->data;
            keyValuePair_t *pairDup = keyValuePair_create(strdup(pair->key), strdup(pair->value));
            list_addElement(apiClient->apiKeys_sessionIdHeaderAuth, pairDup);
        }
    }else{
        apiClient->apiKeys_sessionIdHeaderAuth = NULL;
    }
    if(apiKeys_sessionIdCookieAuth!= NULL) {
        apiClient->apiKeys_sessionIdCookieAuth = list_createList();
        listEntry_t *listEntry = NULL;
        list_ForEach(listEntry, apiKeys_sessionIdCookieAuth) {
            keyValuePair_t *pair = listEntry->data;
            keyValuePair_t *pairDup = keyValuePair_create(strdup(pair->key), strdup(pair->value));
            list_addElement(apiClient->apiKeys_sessionIdCookieAuth, pairDup);
        }
    }else{
        apiClient->apiKeys_sessionIdCookieAuth = NULL;
    }
...
void apiClient_free(apiClient_t *apiClient) {
    ...
    if(apiClient->apiKeys_sessionIdHeaderAuth) {
        listEntry_t *listEntry = NULL;
        list_ForEach(listEntry, apiClient->apiKeys_sessionIdHeaderAuth) {
            keyValuePair_t *pair = listEntry->data;
            if(pair->key){
                free(pair->key);
            }
        ...
        list_freeList(apiClient->apiKeys_sessionIdHeaderAuth);
    }
    if(apiClient->apiKeys_sessionIdCookieAuth) {
        listEntry_t *listEntry = NULL;
        list_ForEach(listEntry, apiClient->apiKeys_sessionIdCookieAuth) {
            keyValuePair_t *pair = listEntry->data;
            if(pair->key){
                free(pair->key);
            }
        ...
        list_freeList(apiClient->apiKeys_sessionIdCookieAuth);
    }
...
void apiClient_invoke(apiClient_t    *apiClient,
    ...
    if(handle) {
        ...
        // this would only be generated for apiKey authentication
        if (apiClient->apiKeys_sessionIdHeaderAuth != NULL)
        {
        list_ForEach(listEntry, apiClient->apiKeys_sessionIdHeaderAuth) {
        keyValuePair_t *apiKey = listEntry->data;
        if((apiKey->key != NULL) &&
        ...
        // this would only be generated for apiKey authentication
        if (apiClient->apiKeys_sessionIdCookieAuth != NULL)
        {
        list_ForEach(listEntry, apiClient->apiKeys_sessionIdCookieAuth) {
        keyValuePair_t *apiKey = listEntry->data;
        if((apiKey->key != NULL) &&
