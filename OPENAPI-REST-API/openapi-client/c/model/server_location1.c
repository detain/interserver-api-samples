...
__attribute__((deprecated)) server_location1_t *server_location1_create(
    ...
    return server_location1_create_internal (
        location_id,
        location_name,
        location_lat,
        location_long,
...
server_location1_t *server_location1_parseFromJSON(cJSON *server_location1JSON){
    ...
    server_location1_local_var = server_location1_create_internal (
        location_id->valuedouble,
        strdup(location_name->valuestring),
        strdup(location_lat->valuestring),
        strdup(location_long->valuestring),
        location_description && !cJSON_IsNull(location_description) ? strdup(location_description->valuestring) : NULL,
        location_ipmi_group ? location_ipmi_group->valuedouble : 0
        );
