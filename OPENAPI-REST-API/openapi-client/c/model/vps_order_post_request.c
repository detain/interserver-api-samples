...
#include <string.h>
#include <stdio.h>
#include "vps_order_post_request.h"
...
static vps_order_post_request_t *vps_order_post_request_create_internal(
    ...
    vps_order_post_request_t *vps_order_post_request_local_var = malloc(sizeof(vps_order_post_request_t));
    ...
    vps_order_post_request_local_var->period = period;
    vps_order_post_request_local_var->location = location;
    vps_order_post_request_local_var->os_version = os_version;
    vps_order_post_request_local_var->hostname = hostname;
    vps_order_post_request_local_var->rootpass = rootpass;
...
void vps_order_post_request_free(vps_order_post_request_t *vps_order_post_request) {
    if(NULL == vps_order_post_request){
        return ;
    }
    if(vps_order_post_request->_library_owned != 1){
        fprintf(stderr, "WARNING: %s() does NOT free objects allocated by the user\n", "vps_order_post_request_free");
        return ;
    }
    ...
    if (vps_order_post_request->os_version) {
        ...
        vps_order_post_request->os_version = NULL;
    }
    if (vps_order_post_request->hostname) {
        free(vps_order_post_request->hostname);
        vps_order_post_request->hostname = NULL;
    }
    if (vps_order_post_request->rootpass) {
        free(vps_order_post_request->rootpass);
        vps_order_post_request->rootpass = NULL;
    }
    if (vps_order_post_request->coupon) {
        free(vps_order_post_request->coupon);
        vps_order_post_request->coupon = NULL;
    }
    if (vps_order_post_request->comment) {
        free(vps_order_post_request->comment);
        vps_order_post_request->comment = NULL;
    }
    free(vps_order_post_request);
}
...
cJSON *vps_order_post_request_convertToJSON(vps_order_post_request_t *vps_order_post_request) {
    ...
    if(cJSON_AddStringToObject(item, "osDistro", vps_order_post_request->os_distro) == NULL) {
    goto fail; //String
    }


    // vps_order_post_request->slices
    ...
    if (interserver_management_api_vps_order_post_request_VPSPLATFORM_NULL == vps_order_post_request->vps_platform) {
        goto fail;
    }
    if(cJSON_AddStringToObject(item, "vpsPlatform", vps_order_post_request_vps_platform_ToString(vps_order_post_request->vps_platform)) == NULL)
    {
    goto fail; //Enum
    }


    // vps_order_post_request->period
    ...
    if(cJSON_AddStringToObject(item, "osVersion", vps_order_post_request->os_version) == NULL) {
    goto fail; //String
    }


    // vps_order_post_request->hostname
    if (!vps_order_post_request->hostname) {
        goto fail;
    }
    if(cJSON_AddStringToObject(item, "hostname", vps_order_post_request->hostname) == NULL) {
    goto fail; //String
    }


    // vps_order_post_request->rootpass
    if (!vps_order_post_request->rootpass) {
        goto fail;
    }
    if(cJSON_AddStringToObject(item, "rootpass", vps_order_post_request->rootpass) == NULL) {
    goto fail; //String
    }


    // vps_order_post_request->controlpanel
    if(vps_order_post_request->controlpanel != interserver_management_api_vps_order_post_request_CONTROLPANEL_NULL) {
    if(cJSON_AddStringToObject(item, "controlpanel", vps_order_post_request_controlpanel_ToString(vps_order_post_request->controlpanel)) == NULL)
    {
    goto fail; //Enum
    }
    }
    ...
    // vps_order_post_request->coupon
    if(vps_order_post_request->coupon) {
    if(cJSON_AddStringToObject(item, "coupon", vps_order_post_request->coupon) == NULL) {
    goto fail; //String
    }
    }


    // vps_order_post_request->comment
    if(vps_order_post_request->comment) {
    if(cJSON_AddStringToObject(item, "comment", vps_order_post_request->comment) == NULL) {
    goto fail; //String
    }
    }
...
vps_order_post_request_t *vps_order_post_request_parseFromJSON(cJSON *vps_order_post_requestJSON){
    ...
    // vps_order_post_request->slices
    cJSON *slices = cJSON_GetObjectItemCaseSensitive(vps_order_post_requestJSON, "slices");
    ...
    interserver_management_api_vps_order_post_request_VPSPLATFORM_e vps_platformVariable;
    
    if(!cJSON_IsString(vps_platform))
    {
    goto end; //Enum
    }
    vps_platformVariable = vps_order_post_request_vps_platform_FromString(vps_platform->valuestring);

    // vps_order_post_request->period
    cJSON *period = cJSON_GetObjectItemCaseSensitive(vps_order_post_requestJSON, "period");
    ...
    // vps_order_post_request->hostname
    cJSON *hostname = cJSON_GetObjectItemCaseSensitive(vps_order_post_requestJSON, "hostname");
    ...
    // vps_order_post_request->rootpass
    cJSON *rootpass = cJSON_GetObjectItemCaseSensitive(vps_order_post_requestJSON, "rootpass");
    ...
    // vps_order_post_request->controlpanel
    cJSON *controlpanel = cJSON_GetObjectItemCaseSensitive(vps_order_post_requestJSON, "controlpanel");
    if (cJSON_IsNull(controlpanel)) {
        controlpanel = NULL;
    }
    interserver_management_api_vps_order_post_request_CONTROLPANEL_e controlpanelVariable;
    if (controlpanel) { 
    if(!cJSON_IsString(controlpanel))
    {
    goto end; //Enum
    }
    controlpanelVariable = vps_order_post_request_controlpanel_FromString(controlpanel->valuestring);
    }

    // vps_order_post_request->coupon
    cJSON *coupon = cJSON_GetObjectItemCaseSensitive(vps_order_post_requestJSON, "coupon");
    ...
    // vps_order_post_request->comment
    cJSON *comment = cJSON_GetObjectItemCaseSensitive(vps_order_post_requestJSON, "comment");
