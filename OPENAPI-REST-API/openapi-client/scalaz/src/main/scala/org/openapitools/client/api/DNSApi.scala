...
import org.openapitools.client.api.DnsListItem
...
object DNSApi {
  ...
  def addDnsRecord(host: String, id: String, name: String, `type`: DnsRecordType, content: String, ttl: Integer = 86400, prio: Integer = 0): Task[Unit] = {
  ...
  def getDnsDomain(host: String, id: Integer): Task[List[DnsRecord]] = {
    implicit val returnTypeDecoder: EntityDecoder[List[DnsRecord]] = jsonOf[List[DnsRecord]]
    ...
    for {
      ...
      uriWithParams =  uri.copy(query = queryParams)
      req           =  Request(method = httpMethod, uri = uriWithParams, headers = headers.put(contentType))
      resp          <- client.expect[List[DnsRecord]](req)

    } yield resp
  }

  def getDnsList(host: String): Task[List[DnsListItem]] = {
    implicit val returnTypeDecoder: EntityDecoder[List[DnsListItem]] = jsonOf[List[DnsListItem]]
    ...
    for {
      ...
      uriWithParams =  uri.copy(query = queryParams)
      req           =  Request(method = httpMethod, uri = uriWithParams, headers = headers.put(contentType))
      resp          <- client.expect[List[DnsListItem]](req)
...
class HttpServiceDNSApi(service: HttpService) {
  ...
  def addDnsRecord(id: String, name: String, `type`: DnsRecordType, content: String, ttl: Integer = 86400, prio: Integer = 0): Task[Unit] = {
    val path = "/dns/{id}".replaceAll("\\{" + "id" + "\\}",escape(id.toString))
  ...
  def getDnsDomain(id: Integer): Task[List[DnsRecord]] = {
    implicit val returnTypeDecoder: EntityDecoder[List[DnsRecord]] = jsonOf[List[DnsRecord]]
  ...
  def getDnsList(): Task[List[DnsListItem]] = {
    implicit val returnTypeDecoder: EntityDecoder[List[DnsListItem]] = jsonOf[List[DnsListItem]]
