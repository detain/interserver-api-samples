...
final api = Openapi().getBackupsApi();
final bool validateOnly = true; // bool | 
final int serviceType = 56; // int | 
final String coupon = coupon_example; // String | 

try {
    final response = api.addBackup(validateOnly, serviceType, coupon);
    print(response);
...
try {
    final response = api.cancelBackup(id);
    print(response);
} on DioException catch (e) {
...
try {
    final response = api.getBackupInfo(id);
    print(response);
} on DioException catch (e) {
...
try {
    final response = api.getBackupInvoices(id);
    print(response);
} on DioException catch (e) {
...
try {
    final response = api.getBackupLogin(id);
    print(response);
} on DioException catch (e) {
...
# **getBackupsList**
> BuiltList<BackupRow> getBackupsList()
...
try {
    final response = api.getBackupsList();
    print(response);
} on DioException catch (e) {
...
[**BuiltList&lt;BackupRow&gt;**](BackupRow.md)
...
try {
    final response = api.getBackupsWelcomeEmail(id);
    print(response);
} on DioException catch (e) {
...
try {
    final response = api.getNewBackup();
    print(response);
} on DioException catch (e) {
...
try {
    final response = api.validateBackupOrder(validateOnly, serviceType, coupon);
    print(response);
} on DioException catch (e) {
