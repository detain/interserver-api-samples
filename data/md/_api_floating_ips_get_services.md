...
## Floating Ips Get Services

### About

**API Function Name: api_floating_ips_get_services**
...
$res = $client
	-> api_floating_ips_get_services($sid);
...
result = client.service.api_floating_ips_get_services()
print result
...
print "got session id ",sid,"\n"
response = client.call(
	:api_floating_ips_get_services, message: {
		sid: ARGV[0],})
print response.body[:api_floating_ips_get_services_response][:return],"\n"
...
try  { 
	...
	if (strlen($sid) == 0)
		die("Got A Blank Session");
	$res = $client->api_floating_ips_get_services($sid);
