...
## Floating Ips Get Client Unpaid Invoices

### About

**API Function Name: api_floating_ips_get_client_unpaid_invoices**
...
$res = $client
	-> api_floating_ips_get_client_unpaid_invoices($sid);
...
result = client.service.api_floating_ips_get_client_unpaid_invoices()
print result
...
print "got session id ",sid,"\n"
response = client.call(
	:api_floating_ips_get_client_unpaid_invoices, message: {
		sid: ARGV[0],})
print response.body[:api_floating_ips_get_client_unpaid_invoices_response][:return],"\n"
...
try  { 
	...
	if (strlen($sid) == 0)
		die("Got A Blank Session");
	$res = $client->api_floating_ips_get_client_unpaid_invoices($sid);
